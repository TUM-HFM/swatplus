      ...
      integer :: iob = 0          !                |
      real :: const = 0.          !                |
      type (hyd_output) :: res_out_combined
...
!!!!! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%res%d == "y") then
            res_out_combined = res_out_d(j)
            res_out_combined%flo = res_out_combined%flo + res_wallo_out_d(j)
            write (2540,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_d(j), res(j), &
                res_in_d(j), res_out_combined, res_wallo_in_d(j), res_wallo_out_d(j)
               if (pco%csvout == "y") then
                 write (2544,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                    res_wat_d(j), res(j), res_in_d(j), res_out_combined, res_wallo_in_d(j), res_wallo_out_d(j)
             end if
          end if
         end if
        ...
        res_wallo_in_m(j)  = res_wallo_in_m(j)  + res_wallo_in_d(j)
        res_wallo_out_m(j) = res_wallo_out_m(j) + res_wallo_out_d(j)
...
!!!!! monthly print
        if (time%end_mo == 1) then
          ...
          res_wallo_in_y(j)  = res_wallo_in_y(j)  + res_wallo_in_m(j)
          res_wallo_out_y(j) = res_wallo_out_y(j) + res_wallo_out_m(j)
          if (pco%res%m == "y") then
            res_out_combined = res_out_m(j)
            res_out_combined%flo = res_out_combined%flo + res_wallo_out_m(j)
            write (2541,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_m(j), res(j), &
                res_in_m(j), res_out_combined, res_wallo_in_m(j), res_wallo_out_m(j)
              if (pco%csvout == "y") then
                write (2545,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                    res_wat_m(j), res(j), res_in_m(j), res_out_combined, res_wallo_in_m(j), res_wallo_out_m(j)
              end if
          end if
          res_in_m(j) = resmz
          res_out_m(j) = resmz
...
!!!!! yearly print
       if (time%end_yr == 1) then
          ...
          res_wallo_in_a(j)  = res_wallo_in_a(j)  + res_wallo_in_y(j)
          res_wallo_out_a(j) = res_wallo_out_a(j) + res_wallo_out_y(j)
          !write (7780,*) res_in_y(j)
          if (pco%res%y == "y") then
            res_out_combined = res_out_y(j)
            res_out_combined%flo = res_out_combined%flo + res_wallo_out_y(j)
            write (2542,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_y(j), res(j), &
                res_in_y(j), res_out_combined, res_wallo_in_y(j), res_wallo_out_y(j)
              if (pco%csvout == "y") then
                write (2546,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                    res_wat_y(j), res(j), res_in_y(j), res_out_combined, res_wallo_in_y(j), res_wallo_out_y(j)
              end if
          end if
          res_in_y(j) = resmz
          res_out_y(j) = resmz
...
!!!!! average annual print
        if (time%end_sim == 1 .and. pco%res%a == "y") then
          ...
          res_wallo_in_a(j)  = res_wallo_in_a(j)  / time%yrs_prt
          res_wallo_out_a(j) = res_wallo_out_a(j) / time%yrs_prt
          res_out_combined = res_out_a(j)
          res_out_combined%flo = res_out_combined%flo + res_wallo_out_a(j)
          write (2543,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_a(j), res(j), &
            res_in_a(j), res_out_combined, res_wallo_in_a(j), res_wallo_out_a(j)
          if (pco%csvout == "y") then
            write (2547,'(*(G0.3,:","))')time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                res_wat_a(j), res(j), res_in_a(j), res_out_combined, res_wallo_in_a(j), res_wallo_out_a(j)
          end if
          res_in_a(j) = resmz
          res_out_a(j) = resmz
          res_wat_a(j) = wbodz
